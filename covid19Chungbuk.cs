    ...
    class covid19Chungbuk
    {
        ...
        public void covid19()
        {
            try
            {
                ...
                _driver.Navigate().GoToUrl("http://www1.chungbuk.go.kr/covid-19/index.do");

                _driver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(10);
                Console.WriteLine("시작");
                IList<IWebElement> e1 = _driver.FindElements(By.ClassName("responsive"))[0].FindElement(By.TagName("tbody")).FindElements(By.TagName("tr"));

                int cnt = e1.Count;

                for (int i = 1; i < cnt; i++)
                {
                    covid19_Chungbuk model = new covid19_Chungbuk();
                    IList<IWebElement> e2 = e1[i].FindElements(By.TagName("td"));
                    model.patient_no = e2[0].Text;
                    model.confirmation_date = e2[1].Text;
                    model.residence = e2[2].Text;
                    model.occurrence_history = e2[3].Text;
                    model.hospital = e2[4].Text;

                    StringBuilder sb = new StringBuilder();
                    sb.Append(" insert into covid19_Chungbuk values(");
